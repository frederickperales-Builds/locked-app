...
import { Dumbbell, Zap, ArrowUp, Layers, RotateCcw, Activity, HeartPulse, ChevronDown, TrendingUp, Lightbulb, ClipboardList, Weight, Link, Flame, Wind, BarChart2, Anchor, Camera, Youtube, Calendar, ChevronLeft, ChevronRight, Download, Upload, ArrowLeftRight, Lock, WifiOff } from "lucide-react";
...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* Tabs */
  .tabs { display: flex; padding: 0 4px; background: var(--card); border-bottom: 1px solid var(--rim); }
  .tab { flex: 1; padding: 8px 0; font-size: 10px; font-weight: 600; color: var(--muted); background: none; border: none; border-bottom: 2px solid transparent; cursor: pointer; transition: color 0.15s, border-color 0.15s; font-family: 'Inter', sans-serif; display: flex; flex-direction: column; align-items: center; gap: 3px; letter-spacing: 0.02em; }
...
const EQUIPMENT_LIST = ["Power Rack", "MX100 Barbell", "Fixed Dumbbells", "Adjustable Dumbbells (REP x Pepin)", "Adjustable Dumbbells (Snode)", "Kettlebells", "Cable Machine", "Preacher Pad", "Butterfly Machine", "Thigh Machine", "Leg Ext/Curl", "Leg Press", "Reverse Hyper", "Resistance Bands", "Accessories"];
...
// Map exercise equipment to machine id
...
function App() {
  ...
  const [workoutName, setWorkoutName] = useState(""); // name of active workout
  ...
  const openStackCalc = (exId, setIdx, currentWeight, equipment) => {
    ...
    setStackMachine(saved?.machine || machineId || "pec-deck");
        ...
        body: JSON.stringify({
          ...
          messages: [{
            ...
            content: [{
              ...
              type: "text",
              text: "This is a workout plan screenshot. Extract the workout name/title and exercises. Return ONLY a JSON object, no markdown.\n\nFormat:\n{\"title\": \"Workout name or title shown (e.g. Upper Weakness Sat 5.9), or Imported Workout if none visible\", \"exercises\": [{\"name\": \"Exercise Name\", \"sets\": 3, \"reps\": \"12-10-8\"}]}\n\nRules:\n- title is the workout title/heading shown in the screenshot\n- name is the exercise name exactly as shown\n- sets is the number of sets (integer)\n- reps is the rep scheme as a string (e.g. 12, 12-10-8, 12-12-12)\n- Return ONLY the JSON object"
            }]
          }]
        })
      });
  ...
  const [equipConfig, setEquipConfig] = useLocalStorage("locked_equip_config", {
    ...
    "Butterfly Machine": "Stack Loaded",
  ...
  const TABS = ["workout", "myworkout", "calendar", "history", "gym"];
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    onClick: () => {
      const LOCKED_KEYS = ["locked_history", "locked_schedule", "locked_cardio_days", "locked_cardio_log", "locked_recovery", "locked_saved_workouts", "locked_preferences", "locked_custom_exercises", "locked_exercise_notes", "locked_bodyweight", "locked_smith_bar", "locked_equip_config", "locked_calc_prefs", "locked_active_plan"];
      const backup = {
        app: "Locked",
        version: "1.0",
    ...
    className: "stack-section-label"
  }, "Main Stack -- ", stackMachineConfig.brand, " (10lb steps)"), /*#__PURE__*/React.createElement("div", {
  ...
  }, pendingIds.includes(pickerExercise.id) ? "✓" : "+")), (pickerExercise.variations || []).map((v, i) => {
